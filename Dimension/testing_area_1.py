...
# Plot the 10D hypercube edges with modified transparency and linewidth
for edge in edges:
    ax.plot3D(*zip(vertices[edge[0]][:3], vertices[edge[1]][:3]), color='red', alpha=0.2, linewidth=1.0)
    ax.plot3D(*zip(vertices[edge[0]][3:6], vertices[edge[1]][3:6]), color='green', alpha=0.2, linewidth=1.0)
    ax.plot3D(*zip(vertices[edge[0]][6:9], vertices[edge[1]][6:9]), color='blue', alpha=0.2, linewidth=1.0)
    ax.plot3D([vertices[edge[0]][9]], [vertices[edge[1]][9]], [0], color='yellow', alpha=0.2, linewidth=1.0)
...
offsets = np.random.uniform(low=-0.2, high=0.2, size=(2**num_dimensions, 3))
