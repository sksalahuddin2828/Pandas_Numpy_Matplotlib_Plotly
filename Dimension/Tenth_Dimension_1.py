...
# Define a mathematical function for the tenth dimension
def tenth_dimension_function(x, y, z):
    return np.sin(x) * np.cos(y) * np.sin(z)

# Apply the function to the tenth dimension
tenth_dimension_values = tenth_dimension_function(vertices[:, 0], vertices[:, 1], vertices[:, 2])
